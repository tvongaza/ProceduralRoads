...
{
    // A crossing is a run of route points over water; anything longer than
    // this is a route that wandered through river core on ordinary moves.
    public const float FordLengthCap = 128f;
    /// <summary>Road floors want this much clearance above the shallow-water
    /// mark; a point below it is over water.</summary>
    public const float WaterlineClearance = 0.75f;
    /// <summary>Water this shallow (bed within this depth of the waterline)
    /// is a ford the road may go through.</summary>
    public const float KneeDeep = 0.8f;
    ...
    public sealed class Report
    {
        ...
        /// <summary>Wet points that no crossing or wading rule covers: the
        /// uncapped dry-land count. The listed violations stop at
    ...
    public static Report Validate(IReadOnlyList<RoadRoute> routes, WorldGenerator world)
    {
        ...
        foreach (RoadRoute route in routes)
        {
            ...
            for (int i = 0; i < route.Points.Count; i++)
            {
                ...
                // Dry-land invariant: a wet point is legal only in river core
                // (a crossing) or as wadeable swamp shallows. Knee-deep water
                // (bed within KneeDeep of the waterline) is a leveled ford by
                // design: the road goes through it.
                bool kneeDeepFord = height >= RoadConstants.SeaLevel - KneeDeep;
                bool exempt = kneeDeepFord || inRiverCore;
                ...
                // cap. Measured on water, not river core: a core band can be a
                // dry valley the road simply runs through.
                bool overWater = height < RoadConstants.ShallowWaterHeight + WaterlineClearance;
                if (overWater)
                {
                    if (fordRunStart < 0)
                        fordRunStart = i;
        ...
        // wet points (WetPoints has those).
