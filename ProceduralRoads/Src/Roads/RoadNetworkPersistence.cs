...
public static class RoadNetworkPersistence
{
    ...
    /// <summary>
    /// Hash key for storing the river crossings (bridges prototype) on the ZDO.
    /// </summary>
        ...
        IReadOnlyCollection<Vector2i> bridgeZones)
    {
        ...
        // Always written, so a network regenerated without bridges does not
    ...
    /// <summary>
    /// Load the river crossings stored with the network (bridges prototype).
    /// A save without the blob (an older build, or a network generated with
    /// bridges off) leaves the list empty.
    /// </summary>
